import { TranslationBookChapter } from './translation-book-chapter.interface';

export const matthew_1_chapter: TranslationBookChapter = {
  translation: {
    id: 'deu_elbbk',
    name: 'Elberfelder Übersetzung (Version von bibelkommentare.de)',
    website: 'https://ebible.org/Scriptures/details.php?id=deuelbbk',
    licenseUrl: 'https://ebible.org/Scriptures/details.php?id=deuelbbk',
    shortName: 'ELBBK',
    englishName: 'German ELBBK Bible',
    language: 'deu',
    textDirection: 'ltr',
    availableFormats: ['json'],
    listOfBooksApiLink: '/api/deu_elbbk/books.json',
    numberOfBooks: 66,
    totalNumberOfChapters: 1189,
    totalNumberOfVerses: 31165,
    languageName: 'Deutsch',
    languageEnglishName: 'German',
  },
  book: {
    id: 'MAT',
    name: 'Matthäus',
    commonName: 'Matthäus',
    title: 'Matthäus',
    order: 40,
    numberOfChapters: 28,
    firstChapterNumber: 1,
    firstChapterApiLink: '/api/deu_elbbk/MAT/1.json',
    lastChapterNumber: 28,
    lastChapterApiLink: '/api/deu_elbbk/MAT/28.json',
    totalNumberOfVerses: 1070,
  },
  chapter: {
    number: 1,
    content: [
      {
        type: 'verse',
        number: 1,
        content: ['Buch des Geschlechts Jesu Christi, des Sohnes Davids, des Sohnes Abrahams.'],
      },
      {
        type: 'verse',
        number: 2,
        content: [
          'Abraham zeugte Isaak; Isaak aber zeugte Jakob, Jakob aber zeugte Juda und seine Brüder;',
        ],
      },
      {
        type: 'verse',
        number: 3,
        content: [
          'Juda aber zeugte Perez und Serach von der Tamar; Perez aber zeugte Hezron, Hezron aber zeugte Aram,',
        ],
      },
      {
        type: 'verse',
        number: 4,
        content: [
          'Aram aber zeugte Amminadab, Amminadab aber zeugte Nachschon, Nachschon aber zeugte Salmon,',
        ],
      },
      {
        type: 'verse',
        number: 5,
        content: [
          'Salmon aber zeugte Boas von der Rahab; Boas aber zeugte Obed von der Ruth; Obed aber zeugte Isai',
          {
            noteId: 0,
          },
          ',',
        ],
      },
      {
        type: 'verse',
        number: 6,
        content: [
          'Isai aber zeugte David, den König. David aber zeugte Salomo von der, die Urias Frau gewesen war;',
        ],
      },
      {
        type: 'verse',
        number: 7,
        content: [
          'Salomo aber zeugte Rehabeam, Rehabeam aber zeugte Abija, Abija aber zeugte Asa,',
        ],
      },
      {
        type: 'verse',
        number: 8,
        content: [
          'Asa aber zeugte Josaphat, Josaphat aber zeugte Joram, Joram aber zeugte Ussija,',
        ],
      },
      {
        type: 'verse',
        number: 9,
        content: ['Ussija aber zeugte Jotham, Jotham aber zeugte Ahas, Ahas aber zeugte Hiskia,'],
      },
      {
        type: 'verse',
        number: 10,
        content: ['Hiskia aber zeugte Manasse, Manasse aber zeugte Amon, Amon aber zeugte Josia,'],
      },
      {
        type: 'verse',
        number: 11,
        content: [
          'Josia aber zeugte Jekonja und seine Brüder zur Zeit der Wegführung nach Babylon.',
        ],
      },
      {
        type: 'verse',
        number: 12,
        content: [
          'Nach der Wegführung nach Babylon aber zeugte Jekonja Schealtiel, Schealtiel aber zeugte Serubbabel,',
        ],
      },
      {
        type: 'verse',
        number: 13,
        content: [
          'Serubbabel aber zeugte Abihud, Abihud aber zeugte Eliakim, Eliakim aber zeugte Azor,',
        ],
      },
      {
        type: 'verse',
        number: 14,
        content: ['Azor aber zeugte Zadok, Zadok aber zeugte Achim, Achim aber zeugte Elihud,'],
      },
      {
        type: 'verse',
        number: 15,
        content: [
          'Elihud aber zeugte Eleasar, Eleasar aber zeugte Matthan, Matthan aber zeugte Jakob,',
        ],
      },
      {
        type: 'verse',
        number: 16,
        content: [
          'Jakob aber zeugte Joseph, den Mann der Maria, von der Jesus geboren wurde, der Christus genannt wird.',
        ],
      },
      {
        type: 'verse',
        number: 17,
        content: [
          'So sind nun alle Geschlechter von Abraham bis auf David 14 Geschlechter, und von David bis zu der Wegführung nach Babylon 14 Geschlechter, und von der Wegführung nach Babylon bis auf den Christus 14 Geschlechter.',
        ],
      },
      {
        type: 'verse',
        number: 18,
        content: [
          'Die Geburt Jesu Christi war aber so: Als nämlich Maria, seine Mutter, dem Joseph verlobt war, wurde sie, ehe sie zusammengekommen waren, schwanger von dem Heiligen Geist.',
        ],
      },
      {
        type: 'verse',
        number: 19,
        content: [
          'Joseph aber, ihr Mann, indem er gerecht war und sie nicht öffentlich zur Schau stellen wollte, gedachte sie heimlich zu entlassen.',
        ],
      },
      {
        type: 'verse',
        number: 20,
        content: [
          'Indem er aber dies bei sich überlegte, siehe, da erschien ihm ein Engel des Herrn',
          {
            noteId: 1,
          },
          'im Traum und sprach: Joseph, Sohn Davids, fürchte dich nicht, Maria, deine Frau, zu dir zu nehmen; denn das in ihr Gezeugte ist von dem Heiligen Geist.',
        ],
      },
      {
        type: 'verse',
        number: 21,
        content: [
          'Und sie wird einen Sohn gebären, und du sollst seinen Namen Jesus',
          {
            noteId: 2,
          },
          'nennen; denn er wird sein Volk erretten von ihren Sünden.',
        ],
      },
      {
        type: 'verse',
        number: 22,
        content: [
          'Dies alles geschah aber, damit erfüllt würde, was von dem Herrn',
          {
            noteId: 3,
          },
          'geredet ist durch den Propheten, der spricht:',
        ],
      },
      {
        type: 'verse',
        number: 23,
        content: [
          '„Siehe, die Jungfrau wird schwanger sein und einen Sohn gebären, und sie werden seinen Namen Emmanuel nennen“,',
          {
            noteId: 4,
          },
          'was übersetzt ist: Gott mit uns.',
        ],
      },
      {
        type: 'verse',
        number: 24,
        content: [
          'Joseph aber, vom Schlaf erwacht, tat, wie ihm der Engel des Herrn befohlen hatte, und nahm seine Frau zu sich;',
        ],
      },
      {
        type: 'verse',
        number: 25,
        content: [
          'und er erkannte sie nicht, bis sie ihren erstgeborenen Sohn geboren hatte; und er nannte seinen Namen Jesus.',
        ],
      },
    ],
    footnotes: [
      {
        noteId: 0,
        caller: '-',
        text: '1,5 Gr. Jessai',
        reference: {
          chapter: 1,
          verse: 5,
        },
      },
      {
        noteId: 1,
        caller: '-',
        text: '1,20 Vergl. 2. Mose 17,9, Anm.',
        reference: {
          chapter: 1,
          verse: 20,
        },
      },
      {
        noteId: 2,
        caller: '-',
        text: '1,21 Vergl. 2. Mose 17,9, Anm.',
        reference: {
          chapter: 1,
          verse: 21,
        },
      },
      {
        noteId: 3,
        caller: '-',
        text: '1,22 „Herr“ ohne Artikel bezeichnet hier und an vielen anderen Stellen den Namen „Jehovas“',
        reference: {
          chapter: 1,
          verse: 22,
        },
      },
      {
        noteId: 4,
        caller: '-',
        text: '1,23 Jes. 7,14',
        reference: {
          chapter: 1,
          verse: 23,
        },
      },
    ],
  },
  thisChapterLink: '/api/deu_elbbk/MAT/1.json',
  thisChapterAudioLinks: {},
  nextChapterApiLink: '/api/deu_elbbk/MAT/2.json',
  nextChapterAudioLinks: {},
  previousChapterApiLink: '/api/deu_elbbk/MAL/3.json',
  previousChapterAudioLinks: {},
  numberOfVerses: 25,
};
